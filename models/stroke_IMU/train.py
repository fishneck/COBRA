...
def get_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--logit_norm",
        action="store_true",
        help="Add --logit_norm option if you want to do logit normalization for cross entropy loss",
    )
    parser.add_argument(
        "--logit_norm_t",
        type=float,
...
def main() -> None:
    ...
    # if you do validation to determine hyperparams
    if config.param_search:
        columns += ["val_loss", "cls_acc", "edit","cer_score"]
        columns += [
    ...
    criterion_cls = ActionSegmentationLoss(
        ...
        logit_norm_t = args.logit_norm_t,
        focal=config.focal,
        tmse=config.tmse,
        ...
        gstmse_weight=config.gstmse_weight,
    )
    ...
    for epoch in range(begin_epoch, config.max_epoch):
        ...
        if config.param_search:
            ...
            if best_edit < edit_score:
                best_edit = edit_score
                torch.save(
                    model.state_dict(),
                    os.path.join(result_path, "best_edit_model.prm"),
                )
                
            if best_cer > cer_score:
                best_cer = cer_score
                torch.save(
                    model.state_dict(),
                    os.path.join(result_path, "best_cer_model.prm"),
                )
            ...
            if best_acc < cls_acc:
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(result_path, "best_acc_model.prm"),
                )
        ...
        if config.param_search:
            # if you do validation to determine hyperparams
            print(
                "epoch: {}\tlr: {:.4f}\ttrain loss: {:.4f}\tval loss: {:.4f}\tval_acc: {:.4f}\tedit: {:.4f}\tcer: {:.4f}".format(
                    epoch,
